...
architecture a of SCOMP is
	type state_type is (
		...
		ex_loadi, ex_add, ex_sub, ex_jpos, ex_jnz, ex_addi,
		...
		if (resetn = '0') then          -- Active-low asynchronous reset
			...
			case state is
				...
				when ex_out2 =>
					IO_WRITE <= '0';
					state <= fetch;
				    -- SUB: AC <= AC - MEM(Address)
					 
				when ex_sub =>
					  AC    <= AC - mem_data;  -- two's complement subtraction
					  state <= fetch;

				 -- JPOS: If AC > 0 (in two's complement), jump.
				 -- "Positive" means sign bit = 0 AND value /= 0.
				when ex_jpos =>
